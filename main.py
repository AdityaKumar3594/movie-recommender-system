...
def get_local_idx_by_title(title: str) -> int:
    ...
    available_titles = list(TITLE_TO_IDX.keys())
    close_matches = get_close_matches(key, available_titles, n=1, cutoff=0.6)
    
    if close_matches:
        return int(TITLE_TO_IDX[close_matches[0]])
...
async def search_bundle(
    ...
    try:
        ...
        # For any other errors, try the query term
        try:
            recs = tfidf_recommend_titles(query, top_n=tfidf_top_n)
        except Exception:
            recs = []
